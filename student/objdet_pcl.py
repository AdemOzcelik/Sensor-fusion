...
def show_pcl(pcl):
    ...
    vis_pcl.create_window(window_name='Open3D', width=1920, height=1080, left=50, top=50, visible=True)
...
def show_range_image(frame, lidar_name):
    ...
    deg90_image = int(img_range_intensity.shape[1] / 4)
    center_image = int(img_range_intensity.shape[1] / 2)
    img_range_intensity = img_range_intensity[:, center_image-deg90_image:center_image+deg90_image]
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    lidar_pcl_cpy[:, 1] = np.int_(np.floor(lidar_pcl_cpy[:, 1] / bev_discrete) + (configs.bev_width + 1) / 2)
